...
def subseqs(s):
    ...
    if not s:
        ...
        seq1 = subseqs(s[1:])
        return insert_into_all(s[0], seq1) + seq1
...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.fund < self.price:
            return 'Please update your balance with ${} more funds.'.format(self.price-self.fund)
        self.store -= 1
    ...
    def restock(self, num):
        self.store += num
        return 'Current {} stock: {}'.format(self.product, self.store)
...
def long_paths(t, n):
    ...
    path = []
    for b in t.branches:
        path += [[t.label] + lst for lst in long_paths(b, n-1)]
    return path
...
def reverse_other(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    branches = t.branches[:len(t.branches)//2]
    reversed_branches = t.branches[::-1][:len(t.branches)//2]
    for b1, b2 in zip(branches, reversed_branches):
