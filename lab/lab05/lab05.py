def flatten(s):
    ...
    """
    "*** YOUR CODE HERE ***"
    type_s = [type(i) != list for i in s]
    if all(type_s):
      return s
    index = [i for i in range(len(s)) if not type_s[i]]
    flattened = []
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dist_a, dist_b = distance(new, city_a), distance(new, city_b)
    if dist_a > dist_b:
