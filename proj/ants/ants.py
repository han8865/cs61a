...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cnt = 0
        while True:
            if place.is_hive or cnt > self.upper_bound:
                return None
            if self.lower_bound <= cnt <=self.upper_bound and place.bees:
    ...
    def throw_at(self, target):
        ...
        def slow_action(gamestate):
            if self.turn < 5:
                ...
                Bee.action(target, gamestate)
        target.action = lambda g: slow_action(g)
        # END Problem EC
    ...
    def insects_in_front(self):
        ...
        while not place.is_hive:
            for bee in place.bees:
                front[bee] = dist
            if place.ant and self != place.ant:
                front[place.ant] = dist
                if place.ant.is_container and place.ant.ant_contained:
                    front[place.ant.ant_contained] = dist
            place = place.entrance
            dist += 1
        return front
        # END Problem Optional 2
