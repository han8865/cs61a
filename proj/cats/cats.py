...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    picked = [i for i in paragraphs if select(i)]
    if k >= len(picked):
        return ''
    return picked[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def find(str):
        std_str = split(lower(remove_punctuation(str)))
        for i in std_str:
            if i in topic: return True
        return False
    return find
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    count = 0
    len_source, len_typed = len(source_words), len(typed_words)
    for i in range(len_source):
        if i < len_typed and typed_words[i] == source_words[i]: count += 1
    if not len_typed: 
        if len_source: return 0.0
        return 100.0
    return 100 * count / len_typed
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff = lambda s: diff_function(typed_word, s, limit)
    word = min(word_list, key=diff)
    return (word if diff(word) <= limit else typed_word)
    # END PROBLEM 5
...
def final_diff(start, goal, limit):
    ...
    """
    The result for final_diff with swap,
    Correction Speed: 120.09909966815572 wpm
    Correctly Corrected: 75 words
    Incorrectly Corrected: 12 words
    Uncorrected: 8 words
    while for minimum_mewtations,
    Correction Speed: 605.7015208277859 wpm
    Correctly Corrected: 297 words
    Incorrectly Corrected: 121 words
    Uncorrected: 38 words
...
def report_progress(typed, prompt, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    progress = 1.0
    if not typed: progress = 0.0
    for i in range(len(prompt)):
        if typed[:(i+1)] != prompt[:(i+1)]:
...
def time_per_word(words, times_per_player):
    ...
    times = [[times_per_player[i][j+1]-times_per_player[i][j] 
            for j in range(len(times_per_player[i])-1)] for i in range(len(times_per_player))]
...
def fastest_words(match):
    ...
    for word in word_indices:
        min_word = time(match, 0, word)
        index = 0
        for player in player_indices:
            pre = time(match, player, word)
            if pre < min_word:
                min_word = pre
                index = player
        fastest[index] += [get_word(match, word)]
    return fastest
    # END PROBLEM 10
